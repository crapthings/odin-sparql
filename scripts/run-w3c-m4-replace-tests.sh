#!/bin/sh
set -eu

root=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
odin_rdf=${ODIN_RDF_COLLECTION:-"$root/../odin-rdf"}
if [ -n "${W3C_M4_REPLACE_SUITE:-}" ]; then
	suite=$W3C_M4_REPLACE_SUITE
else
	syntax_suite=$(sh "$root/scripts/fetch-w3c-tests.sh")
	fixture_root=${syntax_suite%/sparql/sparql11/syntax-query}
	suite="$fixture_root/sparql/sparql11/functions"
fi
runner="$root/.cache/odin-sparql-basic-runner"

odin build "$root/tests/w3c/basic_runner" -out:"$runner" -collection:odin-rdf="$odin_rdf"

total=4
failed=0
for name in replace01 replace02 replace03; do
	if ! "$runner" "$suite/$name.rq" "$suite/data3.ttl" "$suite/$name.srx"; then
		failed=$((failed + 1))
	fi
done
if ! "$runner" "$suite/replace-case-insensitive.rq" /dev/null "$suite/replace-case-insensitive.srx"; then
	failed=$((failed + 1))
fi

printf '%s\n' "W3C SPARQL M4 REPLACE: total=$total failed=$failed"
[ "$failed" -eq 0 ]
