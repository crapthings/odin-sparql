...
if [ -n "${W3C_M4_NOW_SUITE:-}" ]; then
	suite=$W3C_M4_NOW_SUITE
else
...
total=1
failed=0
if ! "$runner" "$suite/now01.rq" "$suite/data.ttl" "$suite/now01.srx"; then
	failed=1
fi

printf '%s\n' "W3C SPARQL M4 NOW subset: total=$total failed=$failed"
[ "$failed" -eq 0 ]
