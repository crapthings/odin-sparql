...
if [ -n "${W3C_M4_BOOLEAN_SUITE:-}" ]; then
  suite=$W3C_M4_BOOLEAN_SUITE
else
  syntax_suite=$(sh "$root/scripts/fetch-w3c-tests.sh")
  fixture_root=${syntax_suite%/sparql/sparql11/syntax-query}
  suite="$fixture_root/sparql/sparql10/optional-filter"
fi
...
do
  total=$((total + 1))
  if ! "$runner" "$suite/expr-$name.rq" "$suite/data-1.ttl" "$suite/expr-$name-result.ttl"; then
    failed=$((failed + 1))
  fi
done

printf '%s\n' "W3C SPARQL M4 boolean subset: total=$total failed=$failed"
[ "$total" -eq 2 ]
[ "$failed" -eq 0 ]
