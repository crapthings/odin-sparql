...
fixtures="$root/tests/construct_modifiers"
...
run_case group-having
run_case group-order-limit
run_case final-values

printf '%s\n' "M6 CONSTRUCT solution modifier gate: total=$total failed=$failed"
[ "$total" -eq 3 ]
[ "$failed" -eq 0 ]
